...
public class CreatePromotionProgramCommand : ICommand
{
...
public class CreatePromotionProgramValidator : AbstractValidator<CreatePromotionProgramCommand>
{
    public CreatePromotionProgramValidator()
    {
        RuleFor(x => x.Name)
            .NotEmpty()
...
public class CreatePromotionProgramHandler(
    IUnitOfWork uow,
    ICacheService cache)
{
    public async Task<Guid> Handle(CreatePromotionProgramCommand command, CancellationToken ct)
    {
        ...
        var promotion = PromotionProgram.Create(
            name: command.Name,
        ...
        uow.PromotionPrograms.Add(promotion);
